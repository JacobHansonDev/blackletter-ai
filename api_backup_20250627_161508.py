...
async def ask_question(
    ...
    try:
        ...
        if doc_item.get('status') != 'completed':
            current_status = doc_item.get('status', 'unknown')
            raise HTTPException(
                status_code=400, 
                detail=f"Document not ready for questions (current status: {current_status})"
            )
        
        # TODO: Replace with actual Q&A pipeline integration
        # For now, generate intelligent mock response based on question
        ...
        logger.error(f"Q&A failed for {user_info['user_id']}: {e}")
